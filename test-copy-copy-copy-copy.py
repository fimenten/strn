        ...
        for epoch in range(self.iterate):
            ...
            for inputs, labels in dataloader:
                ...
                # if self.corr and (not self.base_line) and not ((self.freezing) and (not do_corr)):
                    ...
                    a = 1
                    center = torch.log(weight.mean(dim=a,keepdim=False).detach())
                    center_stacked = torch.stack([center for _ in range(weight.shape[a])],dim = a)

                    # first_loss = first_grad * (weight_raw - center_stacked).detach()/(weight_raw - center_stacked).detach().mean(dim = 1,keepdim = True)
                    ...
                    if False:
                        ...
                        for i in range(weight.shape[1]):
                            for ii in range(i,weight.shape[1]):
                                ...
                                corr_loss = corr_grad*((stacked_w_i_raw - center_stacked)*(stacked_w_ii_raw - center_stacked)).detach()
                                # corr_loss = corr_grad
                                # corrs.append(corr_loss.mean(dim = 0).mean())
                ...
                if (first_loss.mean().sum() < L.mean().mean()) and do_corr :
                    continue
...
if __name__ == "__main__":
    ...
    ds =  [[0,simple_agg,1,1,0,0] for i in range(10)]
    ...
    for d in random.sample(ds,len(ds)): 
        ...
        #                 aggregation=agg,title = str(time.time()),lr_adjusting = 0,freezing=freeze)
        # try:
        #     inst.train()
        # except:
