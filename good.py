        ...
        for epoch in range(self.iterate):
            ...
            for inputs, labels in dataloader:
                ...
                # if self.corr and (not self.base_line) and not ((self.freezing) and (not do_corr)):
                    ...
                    first_loss = first_grad * (weight_raw - torch.log(weight.mean(dim=1,keepdim=True).detach()))
...
if __name__ == "__main__":
    ...
    for d in random.sample(ds,len(ds)): 
        ...
        inst = Experiment_artificial(corr=corr,corr_abs=absolute,lr=0.01,iterate=2000,weighted_learn=weight,
                aggregation=agg,title = str(time.time()),lr_adjusting =0,freezing=freeze,NET = MLP,ensemble_with_simple=False,ensemble_n=8)
