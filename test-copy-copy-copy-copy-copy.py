...
class Experiment:
    def __init__(self,X_train,y_train,X_test,y_test,classify = False,corr = False,corr_abs = False,baseLine = False,
        ...
        self.settings["train_n"] = X_train.shape[0]
        self.settings["test_n"] = X_test.shape[0]
    ...
    def train(self):
        ...
        for epoch in range(self.iterate):
            ...
            first_loss_sum = 0.0
            first_grad_sum = 0.0
            weight_diversity_sum = 0
            second_loss_sum = 0.0
            first_grad_abs_sum = 0
            ii = 0
            if self.freezing:
                do_corr = random.uniform(0,1) > 0.01
                # do_corr = 1
                if do_corr:
            ...
            for inputs, labels in dataloader:
                ...
                do_taylor = random.uniform(0,1) > 0.5
                ...
                # weight = nn.Softmax(dim=abs(1-weight_norm_dim))(weight)
                ...
                # if self.corr and (not self.base_line) and not ((self.freezing) and (not do_corr)):
                    ...
                    # LL = (l - torch.mean(l,dim=1,keepdim=True))
                    # with torch.no_grad():
                    first_grad = torch.zeros_like(weight)
                    if True:
                        for j in range(weight.shape[1]):
                        # first_grad += torch.stack([weight[:,j] for _ in range(weight.shape[1])],dim = 1) *  torch.stack([weight[:,k]*(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1) 
                            # first_grad += weight* torch.stack([weight[:,k]*(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1)
                            # first_grad += weight* torch.stack([(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1)
                            # first_grad += weight* torch.stack([weight[:,k]*(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1)
                            first_grad += weight_raw.detach()* torch.stack([weight[:,k]*(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1)
                            
                            # first_grad += weight* torch.stack([weight[:,k] for k in range(weight.shape[1])],dim=1)
                            
                            # first_grad  +=torch.stack([(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1)

                        # first_grad += torch.stack([weight[:,k]*(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1) 
                        # first_grad += torch.stack([weight[:,k]*(L[:,j] - L[:,k]) for k in range(weight.shape[1])],dim=1)
                    ...
                    # first_loss = first_grad * (weight_raw - torch.log(weight.mean(dim=0,keepdim=True).detach()/weight.sum(dim=0,keepdim=True).detach()))
                    a = 0
                    # center = weight_raw.mean(dim=a,keepdim=False)
                    center = weight.mean(dim=a,keepdim=False)
                    
                    center_stacked = torch.stack([center for _ in range(weight.shape[a])],dim = a).detach()

                    # first_loss = first_grad.detach() * (weight_raw - center_stacked)
                    first_loss = first_grad.detach() * (weight - center_stacked)
                    weight_diversity = (weight - center_stacked)
                    # first_loss = first_grad 
                    ...
                    # first_loss = first_loss.mean(dim = 0).mean()
                    second_grad = torch.zeros_like(weight)
                    # for i in range(weight.shape[1]):
                    #         ii = i
                    if False:
                        # for i,ii in range(i,weight.shape[1]):
                        ...
                        for i in range(weight.shape[1]):
                            for ii in range(i,weight.shape[1]):
                                ...
                                # corr_grad = stacked_w_i * stacked_w_ii * torch.stack([weight[:,k]*(2*L[:,k] - L[:,i] - L[:,ii]) for k in range(weight.shape[1])],dim = 1)
                                # corr_grad = stacked_w_i * stacked_w_ii*(L*2 - torch.stack([L[:,ii] + L[:,i] for _ in range(weight.shape[1])],dim=1))
                                corr_grad = stacked_w_i_raw * stacked_w_ii_raw*(L*2 - torch.stack([L[:,ii] + L[:,i] for _ in range(weight.shape[1])],dim=1))
                                
                                # corr_grad = stacked_w_i*(stacked_w_ii - stacked_w_ii**2)*(L - torch.stack([L[:,ii] for _ in range(weight.shape[1])],dim=1))
                                # corr_loss = corr_grad*(weight_raw - torch.log(weight.mean(dim = 0,keepdim = True)))**2
                                corr_loss = corr_grad.detach()*((stacked_w_i - center_stacked)*(stacked_w_ii - center_stacked).detach())
                                # corr_loss = corr_grad
                                # corrs.append(corr_loss.mean(dim = 0).mean())
                                second_grad += corr_loss
                    second_loss = second_grad
                    # second_loss_sum += second_loss.detach().mean().mean().item()
                    # second_grad = 0
                    # for i in range(weight.shape[1]):
                    #     for ii in range(i,weight.shape[1]):
                    #         stacked_w_i = torch.stack([weight[:,i] for _ in range(weight.shape[1])],dim = 1)
                    ...
                    #         corr_grad = stacked_w_i*(stacked_w_ii - stacked_w_ii**2)*(L - torch.stack([L[:,ii] for _ in range(weight.shape[1])],dim=1))
                    #         # corr_loss = corr_grad*(weight_raw - torch.log(weight.mean(dim = 0,keepdim = True)))**2
                    #         corr_loss = corr_grad*(stacked_w_i_raw - torch.log(weight.mean(dim=1,keepdim=True)))*(stacked_w_ii_raw - torch.log(weight.mean(dim=1,keepdim=True)))
                    #         # corrs.append(corr_loss.mean(dim = 0).mean())
                    #         second_grad += corr_loss
                    # second_loss = second_grad.mean().mean()
                    # second_loss_sum += second_loss.item()
                if y_pred.shape != labels.shape:
                    L = (y_pred - torch.stack([labels for _ in range(y_pred.shape[1])],dim=-1))**2
                    # L = (y_pred - torch.stack([labels for _ in range(y_pred.shape[1])],dim=-1))
                else:                    
                    L = (y_pred - labels)**2
                # loss = torch.mean(nn.Softmax(dim = 0)(-first_loss * first_loss.shape[1]*first_loss.shape[0])  * (criterion(est, labels) * weight) * first_loss.shape[0]).mean()
                if not self.corr:
                    loss = loss_estimate.mean()
                else:
                    # loss = ((L * weight + first_loss - second_loss) * (first_loss/8 - second_loss/8/20 < 0)).mean().mean() + second_loss.abs().mean().mean()
                    # loss = (L * weight).mean().mean() - first_loss.sum(axis = 1).mean()
                    loss = (L * weight - weight_diversity.abs() ).mean().mean()                  
                    # loss = loss_estimate.mean()
                ...
                # if loss_corr:
                #     loss = loss - loss_corr
                # loss = loss_estimate.mean() + (first_loss + second_loss).abs().mean(dim=0).mean()
                # loss = torch.mean(((first_loss) < 0)  * (criterion(est, labels) * weight) ).mean()
                # loss = torch.mean(((first_loss + second_loss) < 0)  * (criterion(est, labels) * weight) ).mean()
                
                # loss = loss_estimate.mean() + first_loss.abs().mean(dim=0).mean() + second_loss.abs().mean(dim=0).mean()
                # loss = loss_estimate.mean() - (first_loss + second_loss).mean(dim=0).mean()
                # loss = (L * weight) * ((L * weight) >first_loss)
                # loss = (loss_estimate).abs().mean() + (L/8**3  - first_loss + second_loss*8).mean().mean().abs()
                # loss = loss_estimate.mean() - (first_loss.mean().mean() + second_loss.mean().mean())
                # loss = loss_estimate.mean() + (first_loss - second_loss).abs().mean().mean()
                # loss = ((L * weight + first_loss - second_loss) * (first_loss/8 - second_loss/8/20 < 0)).mean().mean() + second_loss.abs().mean().mean()
                # loss = ((L * weight + first_loss - second_loss) * (second_loss < 0)).mean().mean()
                
                # loss = ((L * weight + first_loss)).mean().mean()
                
                
                
                # loss = loss.mean().mean()
                
                # loss = ((loss_estimate + (-first_loss))*(L <first_loss*self.ensemble_n**3)).mean().mean()
                # loss = ((loss_estimate + (-first_loss))*(L <first_loss*self.ensemble_n**3)).mean().mean()
                # loss = ((loss_estimate*(L < first_loss) + (-first_loss)*(L > first_loss))).mean().mean()
                # loss = ((loss_estimate + (-first_loss))*(L <first_loss*self.ensemble_n**2)).mean().mean()
                # loss = loss_estimate.mean()

                # if do_taylor:                
                #     loss = (- first_loss).mean(dim = 1).abs().mean()
                # else:
                #     loss = loss_estimate.mean().mean()
                # if y_pred.shape != labels.shape:
                #     # L = (y_pred - torch.stack([labels for _ in range(y_pred.shape[1])],dim=-1)).detach()**2
                #     L = (y_pred - torch.stack([labels for _ in range(y_pred.shape[1])],dim=-1))**2
                # else:                    
                #     # L = (y_pred - labels).detach()**2
                #     L = (y_pred - labels)**2
                # loss = (loss_estimate).mean(dim=0).mean()
                
                
                # loss = loss_estimate.mean() + (first_loss + second_loss.abs()).mean(dim=0).mean()
                
                
                # loss = max(loss_estimate.mean(),-first_loss.mean(dim=0).mean())         
                # loss = loss_estimate.mean() + first_loss.abs().mean(dim=0).mean()
                # loss = loss_estimate.mean() 
                # loss = loss  - (second_loss)
                first_loss_sum += first_loss.mean().mean().item()
                first_grad_sum += first_grad.mean().mean().item()
                first_grad_abs_sum += first_grad.abs().mean().mean().item()
                
                weight_diversity_sum += weight_diversity.abs().mean().mean().item()
                second_loss_sum += second_loss.mean().mean().item()

                total_loss += L.mean().mean().item()
                # with torch.no_grad():
                #     total_loss += criterion(est, labels).mean().mean().item()
                # loss = first_grad.mean().mean() if not do_corr else loss_estimate.mean()
                
                # if (first_loss.mean().mean() < L.mean().mean()) and not do_corr :
                #     continue
                ...
                #     a = 1
                #     center = weight_raw.mean(dim=a,keepdim=False)
                #     center_stacked = torch.stack([center for _ in range(weight.shape[a])],dim = a)

                #     first_loss = first_grad * (weight_raw - center_stacked).detach()
                # if first_loss.mean().mean() < pre_firstloss.mean().mean():
                #     model = pre_model
                #     optimizer = optim.SGD(model.parameters(),lr = learning_rate)
            ...
            if self.classify:
                ...
                with torch.no_grad():
                    ...
                    indicators_d["weighted_testLoss"] = weighted_testLoss.item()
                    indicators_d["new_weighted_testLoss"] = new_weighted_testLoss.item()
                    indicators_d["train_est_loss"] = total_loss/ len(dataloader)
                    indicators_d["train_first_loss"] = first_loss_sum/ len(dataloader)
                    indicators_d["train_first_grad"] = first_grad_sum/ len(dataloader)
                    indicators_d["train_weight_diversity"] = weight_diversity_sum/ len(dataloader)
                    indicators_d["train_first_grad_abs"] = first_grad_abs_sum/ len(dataloader)
                    
                    
                    
                    indicators_d["train_second_loss"] = second_loss_sum/ len(dataloader)
    ...
    def plot_loss_values_2(self,indicators,title,plot_range):
        ...
        for i,c in enumerate(df.drop(columns="epoch").columns):
            # ax.set_ylim(min(plot_range),max(plot_range))
            ax = ax.twinx()
            ax.plot(df[c], label=c, marker='o',color = color_map(i))
            ax.xaxis.set_label("Epoch")
            # ax.yaxis.set_label("Loss")
...
class Experiment_artificial(Experiment):
    def __init__(self, classify=False, corr=False, corr_abs=False, baseLine=False, loss=nn.MSELoss(), lr=0.001, class_n=1, iterate=1000, weighted_learn=True, aggregation=simple_agg, title=str(time.time()), lr_adjusting=False, freezing=False, ensemble_n=8, weight_norm_dim=1, plot_range=(0, 1), negative=True, NET=MLP, ensemble_with_simple=False) -> None:        
        ...
        n_dist = 3
        Data = []
        train_ratios = [0.9 for _ in range(n_dist)]
...
def aaa():
    ...
    for d in random.sample(ds,len(ds)): 
        ...
        inst = Experiment_califolnia_housing(corr=corr,corr_abs=absolute,lr=0.01,iterate=1000,weighted_learn=weight,
                        aggregation=agg,title = str(time.time()),lr_adjusting = 1,freezing=freeze,ensemble_n=32)
        inst.train()
...
if __name__ == "__main__":
    ...
    # ds = [[0,simple_agg,1,1,0,0] for i in range(10)] + [[0,simple_agg,0,1,0,0] for i in range(10)]
    ds = [[1,simple_agg,1,1,0,0] for i in range(10)]

    # ds = [[0,simple_agg,0,0,0,0] for i in range(10)] + [[1,simple_agg,1,0,1,1] for i in range(10)]
    # ds =  [[1,weight_and_pred,0,1,0,0] for i in range(10)]
    ...
    for d in random.sample(ds,len(ds)): 
        ...
        inst = Experiment_artificial(corr=corr,corr_abs=absolute,lr=0.01,iterate=1000,weighted_learn=weight,
                aggregation=agg,title = str(time.time()),lr_adjusting = 1,freezing=freeze,NET = MLP,ensemble_with_simple=False,ensemble_n=8)
        # inst = Experiment_diabate(corr=corr,corr_abs=absolute,lr=0.01,iterate=1000,weighted_learn=weight,
        #                 aggregation=agg,title = str(time.time()),lr_adjusting = 0,freezing=freeze,ensemble_n=8)
        # try:
        #     inst.train()
        # except:
        #     inst.plot(inst.indicators)
        # try:
