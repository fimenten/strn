        ...
        for epoch in range(self.iterate):
            ...
            for inputs, labels in dataloader:
                ...
                # if self.corr and (not self.base_line) and not ((self.freezing) and (not do_corr)):
                    ...
                    # first_loss = first_grad * (weight_raw - center_stacked)
                    ...
                    if False:
                        for i in range(weight.shape[1]):
                            for ii in range(i,weight.shape[1]):
                                ...
                                # corr_loss = corr_grad*(stacked_w_i_raw - center_stacked)*(stacked_w_ii_raw - center_stacked)
                                corr_loss = corr_grad
                                # corrs.append(corr_loss.mean(dim = 0).mean())
                ...
                # loss = loss_estimate.mean() + (-first_loss).mean(dim=0).mean()
                loss = (loss_estimate+first_loss).mean(dim=0).mean()
