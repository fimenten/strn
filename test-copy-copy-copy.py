        ...
        for epoch in range(self.iterate):
            ...
            for inputs, labels in dataloader:
                ...
                # if self.corr and (not self.base_line) and not ((self.freezing) and (not do_corr)):
                    ...
                    #         corr_loss = corr_grad*(stacked_w_i_raw - torch.log(weight.mean(dim=0,keepdim=True)))*(stacked_w_ii_raw - torch.log(weight.mean(dim=0,keepdim=True)))
                    #         # corrs.append(corr_loss.mean(dim = 0).mean())
                ...
                # if loss_corr:
                #     loss = loss - loss_corr
                loss = loss_estimate.mean() - first_loss.mean(dim=0).mean() - second_loss
